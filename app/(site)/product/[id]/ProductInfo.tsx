  ...
  return (
    <div>
      ...
      <p className="text-lg leading-relaxed mb-6 text-[var(--text-secondary)]">
        {product.description ??
          "এই পণ্যের বিস্তারিত বিবরণ শীঘ্রই যোগ করা হবে।"}
      </p>
      ...
      {product.ingredients && (
        <div className="mb-6">
          ...
          <ul className="list-disc list-inside space-y-1 text-lg text-[var(--text-secondary)]">
            {product.ingredients.map((item: string, index: number) => (
              <li key={index}>{item}</li>
            ))}
          </ul>
      ...
      <div className="flex items-center gap-4 mb-8">
        <button
          className="
            w-10 h-10 rounded-lg text-2xl font-bold
            bg-[var(--bg-card)]
